...
public class PhreaticFiltrationModeHTMLPageGenerator {
    private Input input;
    private Output output;
    private String pathInDB = "src/main/resources/BD2.txt";    // путь к файлу, в который сохраняются результаты расчёта и исходные данные
    private String pathForSaveInHTMLFile = "Безнапорная фильтрация.html"; //путь к файлу, в который сохраняется сгенерированный HTML документ
    ...
    public void phreaticFiltrationModeHTMLPageGenerator() {
        ...
        try {
            ...
            output = new Output((getVariableValue(content, "pressureInTheFieldOfNutrition")),
                    ...
                    (getVariableValue(content, "theAverageDepthOfTheFiltrationFloDuringPhreaticFiltration")),
                    (getVariableValue(content, "theReducedRadiusOfTheWaterLoweringSystemSQR")),
                    (getVariableValue(content, "depressionRadiusPhreatic")),
                    ...
                    (getVariableValue(content, "filterRadius")));
        ...
        String htmlContent = "<html>\n" +
                ...
                "</ul>\n" +
                "<h3>2. Расчёт системы водопонижения при безнапорной фильтрации:</h3>\n" +
                "<ul>\n" +
                ...
                "<li>Средняя глубина фильтрационного потока при безнапорной фильтрации:</li>" +
                "h = (2H-S)/2  = (2 ‧ " + dotReplace(output.getPressureInTheFieldOfNutrition()) + " - " + dotReplace(output.getLoweringOfTheGroundwaterLevel()) + ") / 2 = " + dotReplace(output.getTheAverageDepthOfTheFiltrationFloDuringPhreaticFiltration()) + " м\n" +
                ...
                "r<sub>d</sub> = r + 2S<sub>пр</sub> ‧ (k ‧ H)<sup>0,5</sup>  = " + dotReplace(output.getTheReducedRadiusOfTheWaterLoweringSystemSQR()) + " + " + "2" + " ‧ " + dotReplace(output.getLoweringOfTheGroundwaterLevel()) + " ‧ (" + dotReplace(input.getFiltrationCoefficient()) + " ‧ " + dotReplace(output.getPressureInTheFieldOfNutrition()) + ")<sup>0,5</sup>" + " = " + dotReplace(output.getDepressionRadiusPhreatic()) + " м;\n" +
                "<li>Функция понижения от действия водопонизительной системы:</li>" +
                "Ф = (ln(r<sub>d</sub> / r)) / 2 π = (ln(" + dotReplace(output.getDepressionRadiusPhreatic()) + " / " + dotReplace(output.getTheReducedRadiusOfTheWaterLoweringSystemSQR()) + ")) / 2 π" + " = " + dotReplace(output.getLoweringOfWaterFunctionCircle()) + ";\n" +
                "<li>Полный приток подземных вод к системе:</li>" +
                "Q = khS / Ф = " + dotReplace(input.getFiltrationCoefficient()) + " ‧ " + dotReplace(output.getTheAverageDepthOfTheFiltrationFloDuringPhreaticFiltration()) + " ‧ " + dotReplace(output.getLoweringOfTheGroundwaterLevel()) + " / " + dotReplace(output.getLoweringOfWaterFunctionCircle()) + " = " + dotReplace(output.getTotalSystemInflow()) + " м<sup>3</sup>/сут;\n" +
